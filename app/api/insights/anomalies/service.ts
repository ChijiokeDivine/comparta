...
import type { AnomalyStatus, SpendingAnomaly, OnchainTransaction } from "@/app/generated/prisma/client";
...
/**
 * Serializes bigint fields for API responses — including the nested
 * onchainTransaction relation's `amount` when listAnomalies' `include`
 * has pulled it in. Passing a raw bigint anywhere in the response tree
 * to NextResponse.json/JSON.stringify throws ("Do not know how to
 * serialize a BigInt"), so this has to walk one level deep, not just the
 * anomaly's own columns.
 */
export function serializeAnomaly(
  anomaly: SpendingAnomaly & { onchainTransaction?: OnchainTransaction }
) {
  return {
    ...anomaly,
    ...
    ...(anomaly.onchainTransaction
      ? { onchainTransaction: { ...anomaly.onchainTransaction, amount: toDecimalString(anomaly.onchainTransaction.amount) } }
      : {}),
  };
}